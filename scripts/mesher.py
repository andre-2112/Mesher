...
def adaptive_simplify(mesh, target_triangles, curvature_threshold=0.1):
    """Simplify mesh adaptively based on curvature - preserves detail in complex areas."""
    print(f"Adaptive simplification to {target_triangles} triangles (threshold: {curvature_threshold})...")
    ...
    # Calculate curvature as variation in normals at each vertex
    curvatures = np.zeros(len(vertices))
    for i, tri in enumerate(triangles):
        # Get normals of triangle vertices
        n0, n1, n2 = normals[tri[0]], normals[tri[1]], normals[tri[2]]
        # Calculate normal variation (higher = more curvature)
        variation = np.linalg.norm(n0 - n1) + np.linalg.norm(n1 - n2) + np.linalg.norm(n2 - n0)
        # Accumulate to vertices
        for v_idx in tri:
            curvatures[v_idx] += variation
    
    # Normalize curvatures
    if curvatures.max() > 0:
        curvatures = curvatures / curvatures.max()
    
    # High curvature areas get preserved more
    # Use quadric decimation with boundary weight influenced by curvature
    # Note: Open3D doesn't support per-vertex weights directly, so we use a heuristic
    mesh = mesh.simplify_quadric_decimation(
        target_number_of_triangles=target_triangles,
        maximum_error=np.inf,
        boundary_weight=1.0  # Preserve boundaries
    )
    
    print(f"  Adaptive simplification complete: {len(mesh.triangles)} triangles")
    return mesh


def generate_texture(mesh, pcd, uv_method="simple", texture_size=2048):
    """Generate texture from vertex colors with UV mapping."""
    print(f"Generating texture (UV method: {uv_method}, size: {texture_size}x{texture_size})...")
    ...
    try:
        ...
        if vertex_colors is None:
            print("  Warning: No vertex colors to generate texture from")
            return mesh, None
        ...
        if uv_method == "simple":
            # Box projection (fast, simple)
            bounds = tmesh.bounds
            size = bounds[1] - bounds[0]
            
            # Project to dominant plane
            if size[2] < min(size[0], size[1]):  # Flat in Z
                uvs = vertices[:, :2] - bounds[0, :2]
                uvs = uvs / size[:2].max()
            elif size[1] < min(size[0], size[2]):  # Flat in Y
                uvs = vertices[:, [0, 2]] - bounds[0, [0, 2]]
                uvs = uvs / size[[0, 2]].max()
            else:  # Flat in X
                uvs = vertices[:, [1, 2]] - bounds[0, [1, 2]]
                uvs = uvs / size[[1, 2]].max()
        else:
            # Smart unwrap (better quality, slower)
            # Use trimesh's UV unwrapping
            try:
                uvs, faces_uv = trimesh.visual.uv.unwrap(tmesh)
            except:
                print("  Warning: Smart unwrap failed, falling back to simple")
                # Fallback to simple
                bounds = tmesh.bounds
                size = bounds[1] - bounds[0]
                uvs = vertices[:, :2] - bounds[0, :2]
        ...
        texture = Image.new('RGB', (texture_size, texture_size), color=(128, 128, 128))
        pixels = texture.load()
        
        # Sample colors to texture
        for i, uv in enumerate(uvs):
            u = int(np.clip(uv[0], 0, 0.999) * texture_size)
            v = int(np.clip(uv[1], 0, 0.999) * texture_size)
            color = tuple((vertex_colors[i] * 255).astype(np.uint8))
            pixels[u, v] = color
        
        # Add UVs to mesh
        mesh.triangle_uvs = o3d.utility.Vector2dVector(uvs[faces].reshape(-1, 2))
        
        print(f"  ✓ Generated {texture_size}x{texture_size} texture with {uv_method} UV mapping")
        return mesh, texture
        
    except ImportError as e:
        print(f"  Warning: Texture generation requires trimesh and PIL: {e}")
        return mesh, None
...
def post_process_mesh(mesh, pcd=None, cleanup=True, simplify_target=None, simplify_method="uniform", 
                      adaptive_threshold=0.1, fill_holes_size=None, origin_bottom=True, 
                      generate_tex=False, uv_method="simple", texture_size=2048):
    """Post-process mesh with cleanup, simplification, hole filling, and texture generation."""
    ...
    if simplify_target and simplify_target > 0:
        if len(mesh.triangles) > simplify_target:
            if simplify_method == "adaptive":
                mesh = adaptive_simplify(mesh, simplify_target, adaptive_threshold)
            else:  # uniform
                print(f"Uniform simplification to {simplify_target} triangles...")
    ...
    if generate_tex and pcd is not None:
        mesh, texture = generate_texture(mesh, pcd, uv_method, texture_size)
...
Examples:
  ...
  python mesher.py --input_file cloud.ply --output_filename mesh.stl --output_format stl --meshing_method alpha
    ...
    parser.add_argument(
        ...
        help="Simplification method: uniform (standard) or adaptive (preserves detail in high-curvature areas)"
    )
    ...
    parser.add_argument(
        ...
        default=0.1,
        metavar="T",
        help="Curvature threshold for adaptive simplification (default: 0.1, higher = more preservation)"
    )
    
    parser.add_argument(
        "--generate-texture",
        action="store_true",
        help="Generate texture map from vertex colors"
    )
    ...
    parser.add_argument(
        ...
        help="UV mapping method: simple (box projection, fast) or smart (unwrap, better quality)"
    )
    ...
    parser.add_argument(
        ...
        default=2048,
        metavar="SIZE",
        help="Texture resolution (default: 2048x2048)"
    )
